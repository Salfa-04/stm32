...
static void chassis_small_gyro_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector);

static void chassis_small_sin_gyro_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector);
...
{
    static char chassis_RC_last;
    if (chassis_move_mode == NULL)
    {
        return;
    }
    //remote_control_judge(chassis_move_mode);
    if(chassis_RC_last != chassis_move_mode->chassis_RC->rc.s[0])
    {
        chassis_move_mode->control_mode_flag = rc;
    }
    if(chassis_move_mode->control_mode_flag == rc)
    {
        if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))
        {
            chassis_behaviour_mode = CHASSIS_NO_MOVE;
        }
		//地盘跟随云台原本是switch_is_up,/因为把switch_is_mid对应的云台行为模式设置成了绝对角度。
        ...
        {       
            chassis_behaviour_mode = CHASSIS_VECTOR_SMALL_GYRO;
        }
    }
    ...
    {
        chassis_behaviour_mode = CHASSIS_NO_MOVE;
        chassis_move_mode->control_mode_flag = jianshu;//只要有键鼠按下，控制主权就交给键鼠
    }
		//地盘跟随云台原本是switch_is_up,/因为把switch_is_mid对应的云台行为模式设置成了绝对角度。
    else if ( chassis_move_mode->chassis_RC->key.v == KEY_PRESSED_OFFSET_V)
    {
        chassis_behaviour_mode = CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW;
        chassis_move_mode->control_mode_flag = jianshu;
    }
		//小陀螺
	else if ( chassis_move_mode->chassis_RC->key.v == KEY_PRESSED_OFFSET_C)
    {
        chassis_behaviour_mode = CHASSIS_VECTOR_SMALL_GYRO;
        chassis_move_mode->control_mode_flag = jianshu;
    }
    	//正弦小陀螺
    else if ( chassis_move_mode->chassis_RC->key.v == KEY_PRESSED_OFFSET_X)
    {
        chassis_behaviour_mode = CHASSIS_VECTOR_SIN_SMALL_GYRO;
        chassis_move_mode->control_mode_flag = jianshu;
    }
    ...
    if (gimbal_cmd_to_chassis_stop())
    {
     ...
     chassis_RC_last = chassis_move_mode->chassis_RC->rc .s [0];
...
//        chassis_move_mode->chassis_mode = CHASSIS_VECTOR_RAW;
//    }
	else if( chassis_behaviour_mode ==CHASSIS_VECTOR_SMALL_GYRO)
    {
        chassis_move_mode->chassis_mode = CHASSIS_SMALL_GYRO;
    }
    else if( chassis_behaviour_mode ==CHASSIS_VECTOR_SIN_SMALL_GYRO)
    {
        chassis_move_mode->chassis_mode = CHASSIS_SIN_SMALL_GYRO;
    }
}
...
void chassis_behaviour_control_set(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    if (chassis_behaviour_mode == CHASSIS_ZERO_FORCE)//无力
    {
        chassis_zero_force_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
    }
    else if (chassis_behaviour_mode == CHASSIS_NO_MOVE)//不动
    {
    ...
    {
        chassis_small_gyro_yaw_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
    }
    else if( chassis_behaviour_mode ==CHASSIS_VECTOR_SIN_SMALL_GYRO)
    {
        chassis_small_sin_gyro_yaw_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
    }
}
...
/*
    此函数用于控制主权的切换，如果微动开关变动则控制主权为遥控器
*/
//void remote_control_judge(void)
//{
//    if(chassis_RC_last != remote_control->chassis_RC->rc.s[0])
//    {
//        remote_control->control_mode_flag = rc;
//    }
//}
