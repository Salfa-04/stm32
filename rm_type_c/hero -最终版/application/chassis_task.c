...
static void chassis_feedback_update(chassis_move_t *chassis_move_update)
{
    ...
    //yaw的绝对角度—yaw的编码器角度 = 底盘的绝对角度
	chassis_move_update->chassis_yaw = rad_format(*(chassis_move_update->chassis_INS_angle + INS_YAW_ADDRESS_OFFSET) - chassis_move_update->chassis_yaw_motor->relative_angle);
    //（这里因为C板安装不同，把INS_ROLL_ADDRESS_OFFSET和INS_PITCH_ADDRESS_OFFSET互换了一下）
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW)
    {
        ...
        //设置控制相对云台角度//angle_set的值是由遥控器和鼠标确定的。
	...
	else if(chassis_move_control->chassis_mode == CHASSIS_SMALL_GYRO)
    {
        fp32 sin_yaw = 0.0f, cos_yaw = 0.0f;
        //rotate chassis direction, make sure vertial direction follow gimbal 
        //旋转控制底盘速度方向，保证前进方向是云台方向，有利于运动平稳
        //底盘跟随云台模式，将遥控器设置的相对于底盘三个控制量转化为相对于云台的三个控制量
        sin_yaw = arm_sin_f32(chassis_move_control->chassis_yaw_motor->relative_angle);
        cos_yaw = arm_cos_f32(chassis_move_control->chassis_yaw_motor->relative_angle);
        chassis_move_control->vx_set = cos_yaw * vx_set + sin_yaw * vy_set;
        chassis_move_control->vy_set = -sin_yaw * vx_set + cos_yaw * vy_set;
        //set control relative angle  set-point
        //设置控制相对云台角度
        chassis_move_control->chassis_relative_angle_set = rad_format(angle_set);//弧度格式化为-PI~PI
        //calculate ratation speed  
        //计算旋转PID角速度
        //底盘跟随云台，底盘相对与云台应加负号
        //角度期望值为0
    ...
    else if(chassis_move_control->chassis_mode == CHASSIS_SIN_SMALL_GYRO)
    {
        fp32 sin_yaw = 0.0f, cos_yaw = 0.0f;
        //rotate chassis direction, make sure vertial direction follow gimbal 
        //旋转控制底盘速度方向，保证前进方向是云台方向，有利于运动平稳
        ...
        //计算旋转PID角速度
        //底盘跟随云台，底盘相对与云台应加负号
        //角度期望值为SWING_NO_MOVE_ANGLE max_angle ;//* arm_sin_f32(swing_time);
        static fp32 sin_time = 0.0f;
        static fp32 const sin_add_time = 0.002;//PI * 0.5f * configTICK_RATE_HZ / CHASSIS_CONTROL_TIME_MS;
        chassis_move_control->wz_set = angle_set+0.45f* arm_sin_f32(sin_time);
        sin_time += sin_add_time;
        if (sin_time > 2 * PI)
        {   
            sin_time -= 2 * PI;
        }
