...
static void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    if(chassis_move.control_mode_flag == rc)
    {
        ...
        else if (switch_is_mid(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]) || switch_is_up(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]))
        {
    ...
    else if(gimbal_mode_set->gimbal_rc_ctrl->key.v == KEY_PRESSED_OFFSET_V || gimbal_mode_set->gimbal_rc_ctrl->key.v == KEY_PRESSED_OFFSET_C)
    {
        gimbal_behaviour = GIMBAL_ABSOLUTE_ANGLE;//GIMBAL_RELATIVE_ANGLE;
    }
...
static void gimbal_relative_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    *yaw =     yaw_channel * YAW_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN;//如果鼠标移动方向和云台移动方向相反，把第二个负号改成加
    *pitch = pitch_channel * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN;
