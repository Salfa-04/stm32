...
void gimbal_task(void const *pvParameters)
{
    ...
    while (1)
    {
        ...
        shoot_can_set_current = shoot_control_loop();        //射击任务控制循环//弹丸发射（把发射拿出来加上PID）
#if YAW_TURN
        ...
        if (!(toe_is_error(YAW_GIMBAL_MOTOR_TOE) && toe_is_error(PITCH_GIMBAL_MOTOR_TOE) && toe_is_error(TRIGGER_MOTOR_TOE)))
        {
            ...
            {
               CAN_cmd_gimbal(yaw_can_set_current,pitch_can_set_current, shoot_can_set_current, NULL);
                   // CAN_cmd_gimbal(yaw_can_set_current,0, 0, NULL);
//                CAN_cmd_gimbal(0,0, shoot_can_set_current, NULL);
            }
        }
...
{
    gimbal_control.gimbal_yaw_motor.offset_ecd = yaw_offset;//自己给一个编码值
//    gimbal_control.gimbal_yaw_motor.max_relative_angle = max_yaw;//8191
//    gimbal_control.gimbal_yaw_motor.min_relative_angle = min_yaw;//0
...
static void gimbal_init(gimbal_control_t *init)
{
	...
	gimbal_control.gimbal_yaw_motor.offset_ecd = 2746;//1883;//4189;//自己给一个编码值
    gimbal_control.gimbal_yaw_motor.max_relative_angle = 0.8;//max_yaw;//8191;
    gimbal_control.gimbal_yaw_motor.min_relative_angle = -0.8f;//min_yaw;//0;
	gimbal_control.gimbal_pitch_motor.offset_ecd = 3205;//5347;//pitch_offset;
    gimbal_control.gimbal_pitch_motor.max_relative_angle = 0.53000000000f;//max_pitch;
    gimbal_control.gimbal_pitch_motor.min_relative_angle = -0.3224369850f;// min_pitch;
    //修改yaw轴初始yaw目标值。
    ...
    //陀螺仪数据指针获取
    init->gimbal_INT_angle_point = get_INS_angle_point();//获取欧拉角
    init->gimbal_INT_gyro_point = get_gyro_data_point();//获取角速度
    //遥控器数据指针获取
...
static void gimbal_set_control(gimbal_control_t *set_control)
{
    ...
    {
				gimbal_control.motor_mode_flag = 0;
        //gyro模式下，陀螺仪角度控制
    ...
    {
        //gyro模式下，陀螺仪角度控制
				gimbal_control.motor_mode_flag = 1;
...
static void gimbal_absolute_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
    ...
    if (gimbal_motor == NULL)
    {
		...
		if(gimbal_control.motor_mode_flag == 1 )
		{
				//now angle error
				//当前控制误差角度
		...
		if(gimbal_control.motor_mode_flag == 0 )
		{
				angle_set = gimbal_motor->absolute_angle_set;
//    gimbal_motor->absolute_angle_set = rad_format(angle_set + add);
			 gimbal_motor->absolute_angle_set = rad_format(angle_set - add);
...
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
    ...
    //控制值赋值
		...
		else gimbal_motor->given_current = (int16_t)(gimbal_motor->current_set);
}
/**
  * @brief          gimbal control mode :GIMBAL_MOTOR_ENCONDE, use the encode relative angle  to control. 
  * @param[out]     gimbal_motor: yaw motor or pitch motor
...
//			gimbal_motor->given_current = /*-*/(int16_t)(gimbal_motor->current_set);
//		}
//		else if(gimbal_control.motor_mode_flag ==1)
//		{
//			gimbal_motor->given_current = /*-*/-(int16_t)(gimbal_motor->current_set);
//		}
		gimbal_motor->given_current = (int16_t)(gimbal_motor->current_set);
