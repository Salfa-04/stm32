...
#define MOTOR_STATES_BUILD(n, m, io, reset, set)        \
  void Motor##n##_##m##wards(void) {                    \
    HAL_GPIO_WritePin(GPIO##io, reset, GPIO_PIN_RESET); \
    HAL_GPIO_WritePin(GPIO##io, set, GPIO_PIN_SET);     \
  }

#define MOTOR_STATE_BUILD(n, m, io, pin, state)         \
  void Motor##n##_##m(void) {                           \
    HAL_GPIO_WritePin(GPIO##io, pin, GPIO_PIN_##state); \
  }
