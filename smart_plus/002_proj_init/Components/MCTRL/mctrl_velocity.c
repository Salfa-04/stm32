...
void mctrl_chassis_control_velocity(float x, float y, float w) {
  ...
  float vw = w * (CHASSIS_RA + CHASSIS_RB) * 220.f / WHEEL_RADIUS;

  // 速度环控制, 写入目标值 cpm (rpm * 1320)
  PID_MotorA.target = (x - y) * 39600.f / (WHEEL_RADIUS * __PI) - vw;
  PID_MotorB.target = (x + y) * 39600.f / (WHEEL_RADIUS * __PI) + vw;
  PID_MotorC.target = (x - y) * 39600.f / (WHEEL_RADIUS * __PI) + vw;
  PID_MotorD.target = (x + y) * 39600.f / (WHEEL_RADIUS * __PI) - vw;
}

/// Called by TIM Interrupts; 100Hz
