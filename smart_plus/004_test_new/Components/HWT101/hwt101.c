...
void hwt101_event_autocall(void);
...
void hwt101_event_callback(uint16_t size) {
  ...
  hwt101_event_autocall();
}

/// 串口接收回调函数
__weak void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *U, uint16_t Size) {
  if (U->Instance == UART_USED.Instance) hwt101_event_callback(Size);
}
